...
impl Channel {
    ...
    /// Sends a packet
    pub(crate) fn send_to(&mut self, destination_mac:MacAddr, destination_ip: Ipv4Addr, mtu: usize) {
        ...
        let buf = self.packet_builder.build_packet(destination_mac, destination_ip, self.ttl, self.port + self.seq, mtu);
        tx.send_to(&buf, None);
